...
class InfoPane:
    ...
    def draw_pane(self):
        self.score_text = text(self.to_screen(0, 0), self.text_color, self._info_string(0, 1200), "Consolas", self.font_size, "bold")
        self.red_text = text(self.to_screen(230, 0), TEAM_COLORS[0], self._red_score_string(), "Consolas", self.font_size, "bold")
        self.red_text = text(self.to_screen(690, 0), TEAM_COLORS[1], self._blue_score_string(), "Consolas", self.font_size, "bold")
    ...
    def _info_string(self, score, timeleft):
        return "SCORE: % 4d                         TIME:  % 4d" % (score, timeleft)

    def update_score(self, score, timeleft):
        change_text(self.score_text, self._info_string(score, timeleft))
...
class PacmanGraphics:
    def __init__(self, red_team, blue_team, zoom=1.0, frame_time=0.0, capture=False):
        self.expanded_cells = []
        self.have_window = 0
    ...
    def start_graphics(self, state):
        ...
        self.info_pane = InfoPane(layout, self.grid_size, self.red_team, self.blue_team)
    ...
    def update(self, new_state):
        ...
        # dumping food
        if new_state._food_added != None:
            for food_pos in new_state._food_added:
                self.add_food(food_pos, self.food, new_state.layout)
