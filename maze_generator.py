...
class Maze:
    ...
    def add_wall(self, i, gaps=1, vert=True):
        ...
        if vert:
            ...
            self.rooms.append(Maze(self.r, self.c - i - 1,
                                   (add_r, add_c + i + 1), self.root))
        else:
            ...
            self.rooms.append(Maze(self.r - i - 1, self.c,
                                   (add_r + i + 1, add_c), self.root))
...
def make_with_prison(room, depth, gaps=1, vert=True, min_width=1, gapfactor=0.5):
    ...
    room.rooms.append(Maze(room.r, room.c - (2 * p),
                           (add_r, add_c + (2 * p)), room.root))
...
def generate_maze(seed=None):
    ...
    skip = make_with_prison(maze, depth=0, gaps=3,
                            vert=True, min_width=1, gapfactor=gapfactor)
    maze.to_map()
