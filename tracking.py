...
class InferenceModule:
    """
    An inference module tracks a belief distribution over an opponent's location.
    This is an abstract class,
    """
    def __init__(self, opponent_agent_index, distancer, game_state):
        """Sets the opponent agent and distancer for later access"""
    ...
    def get_position_distribution(self, game_state):
        """
        Returns a distribution over successor positions of the opponent from the
        given game_state.
        ...
        for action in legal_actions:
            successor_position = game.Actions.get_successor(opponent_position, action)
    ...
    def set_opponent_position(self, game_state, opponent_position):
        ...
        Note that calling set_opponent_position does not change the position of the
        ghost in the game_state object used for tracking the true progression of
        the game.  It will be a deep copy of the true game_state object.
        """
    ...
    def observe_state(self, game_state, observer):
        """Collects the relevant noisy distance observation and passes it along."""
    ...
    def get_most_likely_position(self):
        """
        Returns the most likely position for the opponent based on the current
        belief state
        """
    ...
    def observe_exact(self, true_position, game_state):
        "Updates beliefs based on the given exact observation and game_state."
        pass
    ...
    def get_belief_distribution(self):
        """
        Returns the agent's current belief state, a distribution over opponent
...
class ExactInference(InferenceModule):
    ...
    def observe_exact(self, position, game_state):
        ...
        for index in self.our_indices:
            if self.distancer.get_distance(game_state.get_agent_position(index),
                             position) < 2.1:
                # POSSIBLY EATEN BY agent
                self.possibly_eaten_by.append((index, game_state.get_agent_position(index)))
    ...
    def observe(self, observation, game_state, observer):
        ...
        """
        if len({key:value for key,value in self.beliefs.items() if value > 0 }) == 0:
            # THIS SHOULD NOT HAPPEN
            print ("***************ALERT: ALL ZEROS********************", self.index)
            # but just in case, we will assume it was because opponent was eaten
        ...
        for p in [p for p in self.legal_positions if self.beliefs[p] > 0]:            
            ...
            all_possible[p] = game_state.get_distance_prob(true_distance, noisy_distance) * self.beliefs[p]                
    ...
    def elapse_time(self, game_state):
        ...
        for pos in self.legal_positions:
            new_pos_dist = self.get_position_distribution(self.set_opponent_position(game_state, pos))
