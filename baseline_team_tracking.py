"""baseline_team_tracking.py

Baseline Team with probabilistic tracking
...
def create_team(first_index, second_index, is_red,
    ...
    """
    inferences = []
    return [eval(first)(first_index, inferences, True), 
            eval(second)(second_index, inferences)]
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def get_features(self, game_state, action):
        ...
        # Computes distance to invaders -- now using the 
        # inferred most likely positions

        enemies = [successor.get_agent_state(i) for i in self.get_opponents(successor)]
        invaders = [a for a in enemies if a.is_pacman]
        invader_indices = [i for i in self.get_opponents(successor) if successor.get_agent_state(i).is_pacman]
        features['num_invaders'] = len(invaders)

        if len(invaders) > 0:
            invader_inferences = []
            for index in invader_indices:
                invader_inferences += [inference for inference in self.inferences if inference.index == index]
            dists = [self.get_maze_distance(my_pos, inference.get_most_likely_position()) 
                        for inference in invader_inferences]
